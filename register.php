...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	...
	if($username == "" || empty($username)){
		header('location: register.php?err=fill-username');
		exit;
	}
	elseif($email == "" || empty($email)){
		header('location: register.php?err=fill-email');
		exit;
	}
	elseif($password == "" || empty($password)){
		header('location: register.php?err=fill-password');
		exit;
	}
	elseif($copassword == "" || empty($copassword)){
		header('location: register.php?err=fill-cofrimpassword');
		exit;
	}
	
	// check username length
	if(strlen($username) > 15){
		header('location: register.php?err=username-toolong');
		exit;
	}
	// too short
	if(strlen($username) < 3){
		header('location: register.php?err=username-tooshort');
		exit;
	}	
	...
	if($c_user > 0){
		...
		header('location: register.php?err=username-exist');
		exit;
	}
	...
	if(strlen($password) < 5){
		header('location: register.php?err=password-tooshort');
		exit;
	}
	...
	if(count($check_email) !== 2){
		header('location: register.php?err=email-invalid');
		exit;
	}else{
	...
	$forgot_key = "";
	// insert to database
	$insertSQL = sprintf("INSERT INTO `user` (email, password, username, reg_date, ver_code, forgot_key, status) VALUES (%s, MD5(%s), %s, %s, %s, %s, %s)",
